export default async function handler(req, res) {
  // accountId, nftDatas, tokenAddrs, tokenIds, offset, limit, nonZero
  const argCheck = req.body.key && req.body.nftDatas && req.body.accountId;
  ...
  if (argCheck && req.body.key === process.env.CUTE_SUNE_API_KEY) {
    ...
    const params = `?accountId=${req.body.accountId}&nftDatas=${req.body.nftDatas}&limit=${limit}`;
    const url = `https://api3.loopring.io/api/v3/user/nft/balances${params}`;
